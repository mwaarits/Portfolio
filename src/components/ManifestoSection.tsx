...
export const ManifestoSection: React.FC = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const progress = total > 0 ? Math.max(0, Math.min(1, -rect.top / total)) : 1;
  ...
  return (
    <section
      id="manifesto"
      ref={sectionRef}
      className="relative h-[300vh] bg-black border-b border-white/10"
    >
      <div className="sticky top-0 h-screen flex items-center px-6 sm:px-12 lg:px-24 overflow-hidden">
        <div className="max-w-6xl mx-auto w-full">
          ...
          <p className="font-heading text-3xl sm:text-5xl md:text-6xl lg:text-7xl font-bold tracking-tight leading-[1.2] select-none">
            {MANIFESTO_WORDS.map((item, index) => {
              const wordThreshold = ((index + 1) / MANIFESTO_WORDS.length) * 0.92;
